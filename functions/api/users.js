...
/**
 * HANDLE GET REQUEST (Mengambil data user, bank, dan statistik)
 */
export async function onRequestGet(context) {
  ...
  try {
    ...
    if (l1.results.length > 0) {
        ...
        // Ambil L2
        // Penggunaan map('?') dan bind(...l1Ids) untuk menghindari SQL injection pada klausa IN
    ...
    return jsonResponse({
      user: {
        ...
        // Saldo sudah ada di safeUser.balance
      },
...
export async function onRequestPost(context) {
    ...
    try {
        if (action === 'update_bank') {
            ...
            // KRITIS: Update kolom bank di tabel users
        ...
        if (action === 'change_pass') {
            ...
            // KRITIS: Update kolom password_hash di tabel users
